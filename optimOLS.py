...
def fconstr3(I):
# defines the constrain of total ammount of installed capacity . Eq. (10) in README.txt.
    fcons=(sum(I[0:Ns])-Isc)**2+(sum(I[Ns:Ns+Nw])-Iwc)**2
    return fcons

def fconstr4(I):    
#define the constrain of the ratio solar to wind technology: rs2w, eq. (11).
    Is=sum(I[:Ns])
    Iw=sum(I[Ns:])
    fcons=0
    if(rs2w>1 and Is<Iw):
...
# defines function to minimize for the OL problem.
    I=X*X
    f0=fun1(I)
    fcons1=fconstr1(I)
    fcons2=fconstr2(I)
...
# defines function to minimize for the OLS problem.
    I=X*X
    f0=fun1(I)
    fcons1=fconstr1(I)
    fcons2=fconstr2(I)
...
def funtryOL(X):
# used to try to verify possible solutions within the constrains of the OL problem
    I=X*X
    fcons1=fconstr1(I)
    fcons2=fconstr2(I)
    fcons3=fconstr3(I)
...
#    print(f0,fcons1,fcons2,fcons3)
    return f
def funtryOLS(X):
# used to try to verify possible solutions within the constrains of the OLS problem
    I=X*X
    fcons1=fconstr1(I)
    fcons2=fconstr2(I)
    fcons4=fconstr4(I)
...
if(ff > 1.e-11):
    ...
    for i in range(len(IminR)):
        if(i<Ns):
            ...
            ii=i+1-Ns
        if(I[i] < IminR[i]):
            print(Ty+' region:',ii,'   I[i]=%6.3f should be > Imin[i]=%6.3f' %(I[i],IminR[i]))
        if(I[i] > ImaxR[i]):
            print(Ty+' region:',ii,'   I[i]=%6.3f should be > Imax[i]=%6.3f' %(I[i],ImaxR[i]))
    print()
